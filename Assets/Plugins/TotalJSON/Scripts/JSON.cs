...
namespace Leguar.TotalJSON {
	...
	public class JSON : JValue {
		...
		private readonly Dictionary<string,JValue> keyValuePairs;
		...
		/// <summary>
		/// Returns compact information of this JSON object as string, for debug purposes.
		///
		/// When you want to get all content of this JSON object in JSON formatted string, use <see cref="CreateString()"/>.
		/// </summary>
		/// <returns>
		/// Single string with information of this JSON object.
		/// </returns>
		...
		///
		/// Note that for large JSON objects output may be also very long. For debugging purposes, consider also <see cref="ToString()"/>.
		/// </summary>
		/// <returns>
		...
		internal override void zCreate(CreateStringRunner createStringRunner) {
			...
			if (keyValuePairs.Count>0) {
				...
				foreach (string key in keyOrder) {
					...
					JString.encode(createStringRunner,key,false);
		...
		public static JSON ParseString(string jsonObjectAsSingleString, ParseStringSettings parseStringSettings) {
			...
			if (parseStringSettings==null) {
				throw (new JArgumentNullException("parseStringSettings","Settings can not be null in JSON.ParseString(string,ParseStringSettings)"+ParseException.getExceptionMessageTail(null)));
			}
			return innerParseString(jsonObjectAsSingleString,parseStringSettings);
		}
		...
		internal static JSON zParse(ParseStringRunner parseStringRunner, bool expectStartOfObject, bool allowEndOfString, bool allowTrashInsteadObjectStart) {
			...
			if (expectStartOfObject) {
				...
				if (chr!='{') {
					...
					throw ParseException.forInvalidStart("Invalid character "+InternalTools.logSafeCharacter(chr)+" when expecting start of object '{'", parseStringRunner);
				}
			}
			...
			do {
				if (chr!='"') {
					throw ParseException.forInvalidCharacter("Invalid character "+InternalTools.logSafeCharacter(chr)+" when expecting start of key string '\"'", parseStringRunner);
				}
				string keyString=JString.decode(parseStringRunner);
				chr=sp.getNextNonWhiteChar();
				if (chr!=':') {
					throw ParseException.forInvalidCharacter("Invalid character "+InternalTools.logSafeCharacter(chr)+" when expecting key/value separator ':'", parseStringRunner);
				}
				...
				if (chr!=',') {
					throw ParseException.forInvalidCharacter("Invalid character "+InternalTools.logSafeCharacter(chr)+" when expecting pair separator ',' or end of object '}'", parseStringRunner);
				}
				chr=sp.getNextNonWhiteChar();
			} while (true);
